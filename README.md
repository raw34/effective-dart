...
- 类
    - [避免] [避免定义单成员抽象类使用函数替代](design.md#3-1)
    - [避免] [避免定义只含一个静态成员的类](design.md#3-2)
    - [避免] [避免继承不想拥有子类的类](design.md#3-3)
    - [要] [如果你的类支持继承请在文档里说明](design.md#3-4)
    - [避免] [避免实现不支持作为接口的类](design.md#3-5)
    - [要] [如果你的类支持作为借口请在文档里说明](design.md#3-6)
    - [避免] [避免混合`mixin`不支持作为`mixin`的类](design.md#3-7)
    - [要] [如果你的类支持作为`mixin`请在文档里说明](design.md#3-8)
- 构造函数
    - [推荐] [建议定义构造函数而不是静态方法去生成实例](design.md#4-1)
    - [考虑] [如果类支持请将构造函数声明为`const`](design.md#4-2)
- 类成员
    - [推荐] [将顶级作用域的变量声明为`final`](design.md#4-1)
    - [要] [为可获得的属性添加`getters`](design.md#4-2)
    - [要] [为可设置的属性添加`setters`](design.md#4-3)
    - [不要] [不要定义`setters`时不定义对应的`getters`](design.md#4-4)  
    - [避免] [避免返回值返回`null`](design.md#4-5)
    - [避免] [避免直接返回`this`,链式调用请用cascade也就是`..`运算符](design.md#4-6)
- 类型
    - [推荐] [声明顶级作用域变量类型](design.md#5-1)
    - [考虑]  [声明私有作用域变量类型](design.md#5-2)
    - [避免] [避免声明初始化本地变量类型](design.md#5-3)
    - [避免] [避免在闭包中声明推断参数的类型](design.md#5-4)
    - [避免] [避免在使用泛型时重复冗余的声明类型](design.md#5-5)
    - [要] [当推断类型时确定类型时请声明类型](design.md#5-6)
    - [推荐] [使用`dynamic`声明类型而不是让推断直接fail掉](design.md#5-7) 
    - [推荐] [当函数作为参数时请声明函数类型](design.md#5-8)
    - [不要] [不要为`setter`声明类型](design.md#5-9)
    - [不要] [不要使用遗留版本的`typedef`语法](design.md#5-10)
    - [推荐] [使用函数类型声明而不是`typedef`](design.md#5-11) 
    - [考虑] [函数作为参数时使用函数类型语法](design.md#5-12) 
    - [要] [当参数可以是任意类型时用`Object`声明而不是`dynamic`](design.md#5-13)
    - [要] [当异步函数不返回值时使用`Future<void>`声明](design.md#5-14)
    - [避免] [避免使用`FutureOr<T>`作为返回类型](design.md#5-15)
- 参数
    - [避免] [避免布尔值类型位置参数](design.md#6-1)
    - [避免] [如果你想省略一些参数请请避免使用位置参数](design.md#6-2)
    - [避免] [避免强制性参数当参数可以省略时](design.md#6-3)
    - [要] [获取范围时将参数设置为左闭右开](design.md#6-4)
- 相等符
    - [要] [如果你重载`==`请一并重载`hashcode`](design.md#7-1)
    - [要] [请让你的`==`运算符遵守数学上的相等](design.md#7-2)
    - [避免] [避免为可变类定义常规意义上的相等](design.md#7-3)
    - [不要] [不需要在重载`==`运算符时判断类型是否为`null`](design.md#7-4)
